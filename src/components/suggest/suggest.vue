  ...
  export default {
    ...
    methods: {
      ...
      searchMore() {
        ...
        search(this.query, this.page, this.showSinger, perpage).then((res) => {
          if (res.code === ERR_OK) {
           //  var readyP = Promise.resolve(this._genResult(res.data))
           // readyP.then( (data) => {
           //   this.result = this.result.concat(data)
           //   this._checkMore(res.data)
           // })
              let newdata = this._genResult(res.data)
            setTimeout(() =>{
              this.result = this.result.concat(newdata)
              this._checkMore(res.data)
            }, 700)
      ...
      selectItem(item) {
        ...
        this.$emit('select', item)  //派发事件让外层组件监听 可以保存历史记录
      },
      getDisplayName(item) {
        if (item.type === TYPE_SINGER) {
      ...
      _normalizeSongs(list) {
        let ret = []
        list.forEach((song) => {
          getDiscSongVkey(song.songmid).then((res) => {
            if (res.code === ERR_OK) {
